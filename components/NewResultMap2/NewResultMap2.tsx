...
import { StyledNewResultMap2 } from '.'
...
const NewResultMap2: FC<Props> = ({ guessedLocations, actualLocations, round, isFinalResults }) => {
  ...
  const generateLineData = (guessedLoc: GuessType, actualLoc: LocationType) => {
    return {
      ...
      features: [
        {type: 'Feature', geometry: {type: 'LineString', coordinates: [[guessedLoc.lng, guessedLoc.lat], [actualLoc.lng, actualLoc.lat]]}}
      ]
    } as any
  }
  ...
  return (
    <StyledNewResultMap2>
      ...
      >
        ...
        {isFinalResults && guessedLocations.map((guessedLoc, idx) => (  
          <Marker key={idx} longitude={guessedLoc.lng} latitude={guessedLoc.lat}>
            <img className="userMarker" src={`/images/avatars/default3.jpg`} alt="User Map Pin" />
          </Marker>       
        ))}

        {isFinalResults && actualLocations.map((actualLoc, idx) => (
        ...
        {isFinalResults && actualLocations.map((actualLoc, idx) => (
           <Source key={idx} id={`finalsResult-${idx + 1}`} type="geojson" data={generateLineData(guessedLocations[idx], actualLoc)}>
            <Layer id={`finalResult-${idx + 1}`} {...layerStyle} />
           </Source>
        ))}
        <Source id="results" type="geojson" data={generateLineData(guessedLocations[0], actualLocations[0])}>
              <Layer {...layerStyle} />
            </Source>
      </Map>
    </StyledNewResultMap2>
